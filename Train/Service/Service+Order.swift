...
// MARK: - Request Flow
    ...
    func preOrderFlow(isAuto:Bool,success:@escaping (UIImage)->Void,failure: @escaping (NSError)->Void){
        self.getPassengerDTOs(isAuto: isAuto).then{_ -> Promise<UIImage> in
...
// MARK: - Chainable Request
    ...
    func getPassCodeNewForPassenger()->Promise<UIImage>{
        return Promise{ fulfill, reject in
